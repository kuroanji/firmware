...
void Events::syncNodes() {
    ...
    for (auto it = nodeDB->meshNodes->begin(); it != nodeDB->meshNodes->end(); ++it) {
        ...
        // Position info
        // NOTE: positions no longer live inside NodeInfoLite — they are held in a
        // separate table (nodeDB->nodePositions) and fetched via copyNodePosition().
        meshtastic_PositionLite theirPos;
        entry.hasPosition = nodeDB->hasValidPosition(node) && nodeDB->copyNodePosition(node->num, theirPos);
        if (entry.hasPosition) {
            entry.latitude = theirPos.latitude_i;
            entry.longitude = theirPos.longitude_i;
        }
        ...
        if (node->short_name[0] != '\0') {
            strncpy(entry.shortName, node->short_name, sizeof(entry.shortName) - 1);
        ...
        if (node->long_name[0] != '\0') {
            strncpy(entry.longName, node->long_name, sizeof(entry.longName) - 1);
        ...
        if (mapModule && entry.hasPosition) {
            ...
            // is_favorite moved into NodeInfoLite::bitfield upstream
            mapEntry.isFavorite = (node->bitfield & NODEINFO_BITFIELD_IS_FAVORITE_MASK) != 0;
    ...
    if (mapModule && ourNode && nodeDB->hasValidPosition(ourNode)) {
        meshtastic_PositionLite ourPos;
        if (nodeDB->copyNodePosition(ourNode->num, ourPos)) {
            mapModule->setOwnPosition(ourPos.latitude_i, ourPos.longitude_i);
        }
    }
}
...
int Events::beforeDeepSleep(void* unused) {
    ...
    // Backup NodeDatabase before shutdown (for recovery if main file corrupts)
    nodeDB->backupNodeDatabase();
...
int Events::beforeReboot(void* unused) {
    ...
    // Backup NodeDatabase before reboot (for recovery if main file corrupts)
    nodeDB->backupNodeDatabase();
