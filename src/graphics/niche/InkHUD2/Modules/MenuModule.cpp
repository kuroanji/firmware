...
void MenuModule::renderStatusBar(RenderContext& ctx) {
    ...
    if (rtcTime > 0) {
        ...
        // Use gmtime because getValidTime(..., true) already applies TZ offset
...
void MenuModule::renderAlert(RenderContext& ctx) {
    ...
    // Draw alert message (wrapped with height limit to avoid overlapping footer)
    if (alertMessage) {
        uint16_t textInset = layout->textInset();
        uint16_t topPad = layout->elementSpacing();
        uint16_t availableHeight = content.h > topPad ? content.h - topPad : content.h;
        ctx.textWrappedTruncated(content.left() + textInset, content.top() + topPad,
                                  content.w - 2 * textInset, availableHeight,
                                  alertMessage, Color::BLACK);
    }
}
...
void MenuModule::renderShutdownFinal(RenderContext& ctx) {
    ...
    if (ourNode && ourNode->short_name[0] != '\0') {
        int16_t textY = logoCY + logoH / 2 + layout->lineHeight();
        ctx.text(centerX, textY, ourNode->short_name, Align::CENTER, Color::BLACK);
    }
}
