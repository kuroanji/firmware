...
void BootModule::renderLogo(RenderContext& ctx) {
    ...
    if (ourNode && ourNode->short_name[0] != '\0') {
        ctx.text(w - padding, padding, ourNode->short_name, Align::RIGHT, Color::BLACK);
    }
...
void BootModule::renderPairing(RenderContext& ctx) {
    ...
    meshtastic_NodeInfoLite *ourNode = nodeDB->getMeshNode(nodeDB->getNodeNum());
    if (ourNode && ourNode->short_name[0] != '\0') {
        ctx.textScaled(centerX, nameY, ourNode->short_name, textScale, Align::CENTER, Color::BLACK);
    }
}
