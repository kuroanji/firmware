...
public:
    ...
    void setIdleFullRefresh(uint32_t ms);  // 0 = disabled
    void setBacklightIdleOff(uint32_t ms); // 0 = disabled; auto-off backlight after idle
...
private:
    ...
    // Idle maintenance (optional, for displays with ghosting issues)
    uint32_t idleFullRefreshMs = 0;       // 0 = disabled
    uint32_t lastUserInputTime = 0;       // Last button press
    bool lastRefreshWasFull = true;       // Track refresh type
    bool idleMaintenanceDone = false;     // One-shot per idle period

    // Backlight auto-off (optional; for devices whose light would otherwise idle on)
    uint32_t backlightIdleOffMs = 0;      // 0 = disabled
    bool backlightAutoOffDone = false;    // One-shot per idle period
};
