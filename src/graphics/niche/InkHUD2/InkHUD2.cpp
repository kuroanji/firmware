...
#include "graphics/niche/Drivers/Backlight/LatchingBacklight.h"  // for backlight auto-off
...
int32_t InkHUD2::runOnce() {
    ...
    // Idle maintenance: FULL refresh after idle to clear ghosting (one-shot)
    if (idleFullRefreshMs > 0 && !idleMaintenanceDone && !lastRefreshWasFull) {
        uint32_t now = millis();
        if (now - lastUserInputTime > idleFullRefreshMs) {
            fullRefreshRequested = true;
            idleMaintenanceDone = true;  // Don't repeat until next user input
        }
    }

    // Backlight auto-off after idle (one-shot).
    // For devices where the light has no zero position of its own (e.g. ThinkNode M1,
    // whose knob dims but never fully extinguishes), so a light left on would drain the
    // battery. Turning it off here is safe: any user input turns it back on and resets
    // this, see onInput().
    if (backlightIdleOffMs > 0 && !backlightAutoOffDone) {
        if (millis() - lastUserInputTime > backlightIdleOffMs) {
            NicheGraphics::Drivers::LatchingBacklight* bl =
                NicheGraphics::Drivers::LatchingBacklight::getInstance();
            if (bl->isOn())
                bl->off();
            backlightAutoOffDone = true;  // Don't repeat until next user input
        }
    }
...
void InkHUD2::onInput(Input input) {
    ...
    // Track user activity for idle maintenance
    lastUserInputTime = millis();
    idleMaintenanceDone = false;  // Reset for next idle period

    // Any input wakes the backlight (if auto-off is in use) and re-arms the timer
    if (backlightIdleOffMs > 0) {
        backlightAutoOffDone = false;
...
void InkHUD2::render() {
    ...
    // Track for idle maintenance
    lastRefreshWasFull = doFullRefresh;
...
void InkHUD2::setIdleFullRefresh(uint32_t ms) {
    idleFullRefreshMs = ms;
}

void InkHUD2::setBacklightIdleOff(uint32_t ms) {
    backlightIdleOffMs = ms;
    if (ms > 0) {
        // Start lit, then let the idle timer put it out. Also arms the timer from boot,
        // so an untouched device goes dark on its own.
        lastUserInputTime = millis();
        backlightAutoOffDone = false;
        NicheGraphics::Drivers::LatchingBacklight::getInstance()->latch();
    }
