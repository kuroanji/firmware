...
static std::function<void()> onGPSChange;
...
void setup(NicheGraphics::Drivers::EInk* driver, const Config& config)
{
    ...
    // rotation is OFFSET from device default (0=default, 1=+90°, 2=+180°, 3=+270°)
    uint8_t rotationOffset = Settings::instance().getRotation();
    uint8_t initialRotation = (rotationOffset + config.defaultRotation) % 4;
    ...
    // Apply idle maintenance setting (for displays with ghosting issues)
    if (config.idleFullRefreshMs > 0) {
        hud.setIdleFullRefresh(config.idleFullRefreshMs);
    }
    ...
    onGPSChange = []() {
        ...
        if (::config.position.gps_mode == meshtastic_Config_PositionConfig_GpsMode_NOT_PRESENT) {
            gpsEnabled = false;  // Reset toggle
            menuModule->showAlert("No GPS module on this device");
            return;
        }
        // Toggle GPS mode
        if (gpsEnabled) {
    ...
    onHidePINChange = []() {
        ...
        // hidePIN now lives in InkHUD2's own settings file, not in Meshtastic
        // config (upstream removed config.bluetooth.hide_pin) — so persist via
        // Settings::save(), not nodeDB->saveToDisk().
        auto& settings = Settings::instance();

        if (settings.getHidePIN()) {
            if (!isFixedPIN) {
                settings.setHidePIN(false);
                menuModule->showAlert("Only for Fixed PIN mode");
                return;
            }
            // Save and show result
            settings.save();
            menuModule->showAlert("PIN hidden. To show: set Random PIN in app.");
        } else {
            if (isFixedPIN) {
                settings.setHidePIN(true);
                settings.save();
                menuModule->showAlert("PIN hidden. To show: set Random PIN in app.");
                return;
            }
    ...
    onRotationChange = []() {
        // rotation is OFFSET from device default (0=default, 1=+90°, 2=+180°, 3=+270°)
        uint8_t offset = Settings::instance().getRotation();
        uint8_t bufferRotation = (offset + currentConfig.defaultRotation) % 4;
    ...
    // Initialize GPS state from config
    gpsEnabled = (::config.position.gps_mode == meshtastic_Config_PositionConfig_GpsMode_ENABLED);

    mainMenuItems[0].label = "GPS";
    mainMenuItems[0].type = MenuItemType::TOGGLE;
    mainMenuItems[0].toggleValue = &gpsEnabled;
    mainMenuItems[0].onChange = &onGPSChange;
    ...
    if (config.hasBacklight && config.backlightPin >= 0) {
        ...
        // Auto-off must be armed AFTER setPin(): it lights the backlight immediately,
        // and LatchingBacklight asserts that a pin has been assigned.
        if (config.backlightIdleOffMs > 0)
            hud.setBacklightIdleOff(config.backlightIdleOffMs);
    }
    ...
    if (config.hasAuxButton && config.auxButtonPin >= 0) {
        ...
        // Only spend the aux button on the backlight if the backlight needs a button.
        // With auto-off enabled the light manages itself (on with any input, off when
        // idle), so the button is better used for scrolling.
        if (config.hasBacklight && config.backlightIdleOffMs == 0) {
            // Backlight control
            Drivers::LatchingBacklight* backlight = Drivers::LatchingBacklight::getInstance();
